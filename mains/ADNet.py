...
from trainers.adnet_train_rl_mot import adnet_train_rl_mot

sys.path.insert(0, '../')
from trainers.adnet_test_sl import adnet_test_sl
from trainers.adnet_train_sl_only_score import adnet_train_sl_only_score

from trainers.adnet_train_sl_mot import adnet_train_sl_mot
...
def do():
    ...
    parser.add_argument('--adnet_mot', default=False, type=str, help='Whether to test or train.')

    parser.add_argument('--test', default=False, type=str, help='Whether to test or train.')
    # parser.add_argument('--resume', default=None, type=str, help='Resume from checkpoint')
    parser.add_argument('--mot', default=False, type=bool, help='Perform MOT tracking')
    parser.add_argument('--resume', default='weights/ADNet_RL_FINAL.pth', type=str, help='Resume from checkpoint')
    ...
    parser.add_argument('--send_images_to_visualization', type=str2bool, default=False,
                        help='Sample a random image from each 10th batch, send it to visdom after augmentations step')
    parser.add_argument('--save_folder', default='weights', help='Location to save checkpoint models')

    parser.add_argument('--save_file', default='ADNet_SL_MOT', type=str, help='save file part of file name for SL')
    ...
    parser.add_argument('--run_supervised', default=False, type=str2bool,
                        help='Whether to run supervised learning or not')

    parser.add_argument('--multidomain', default=True, type=str2bool,
                        help='Separating weight for each videos (default) or not')

    parser.add_argument('--save_result_images', default=False, type=str2bool,
    ...
    if args.run_supervised:
        ...
        if args.test:
            ...
            if args.adnet_mot:
                _, _, train_videos = adnet_train_sl_mot(args, opts, mot=args.mot)
            else:
        ...
        if False and args.start_iter == 0:  # means the weight came from the SL
            net, domain_specific_nets = adnet_mot(opts, trained_file=args.resume,
    ...
    if True:
        ...
        if args.adnet_mot:
            net = adnet_train_rl_mot(net, domain_specific_nets, train_videos, opts, args, 2)
        else:
